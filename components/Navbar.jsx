...
export default function Navbar({ minimal = false, active = 0, setActive }) {
  ...
  if (minimal) {
    return (
      <nav className="fixed top-0 left-0 w-full z-50 h-16 flex items-center justify-between px-6 bg-[#0a0a13]/90 backdrop-blur-md">
          ...
          {navLinks.slice(0, 4).map((link, i) => (
            link.href !== "#footer" && (
              <a
                ...
                className={`relative px-4 py-2 rounded-lg font-medium transition-all duration-300 border border-transparent focus:outline-none focus:ring-2 focus:ring-fuchsia-400
                  ${active === i
                    ? "bg-gradient-to-r from-purple-500 via-fuchsia-500 to-blue-500 text-white shadow-[0_0_16px_2px_rgba(168,85,247,0.4)] border-fuchsia-400"
